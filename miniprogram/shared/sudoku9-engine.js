...
function makeSolution(rng = Math.random) {
  ...
  const rows = rowBands.flatMap((band) => shuffle([0, 1, 2], rng).map((row) => band * BOX_SIZE + row));
  const colBands = shuffle([0, 1, 2], rng);
  const cols = colBands.flatMap((band) => shuffle([0, 1, 2], rng).map((col) => band * BOX_SIZE + col));
...
function finalizeFailure(state, now = Date.now()) {
  ...
  stopped.won = false;
  stopped.paused = false;
  stopped.lastMessage = "失败：明错误已超过 3 次，只能重新开局。";
...
function placeNumber(state, number, now = Date.now()) {
  ...
  if (hasConflict) {
    ...
    next.lastMessage = `这一行、列或九宫格里已经有 ${number}，错误 ${next.mistakes}/${next.maxMistakes}。`;
...
function createBoardCells(state) {
  ...
  return state.current.map((value, index) => {
    ...
    return {
      ...
      noteRows,
      boxRight: (index % GRID_SIZE) === 2 || (index % GRID_SIZE) === 5,
      boxBottom: (Math.floor(index / GRID_SIZE)) === 2 || (Math.floor(index / GRID_SIZE)) === 5
    };
  });
}
