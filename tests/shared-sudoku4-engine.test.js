...
const engine = require("../miniprogram/shared/sudoku4-engine");
...
test("4x4 createNewGame builds a valid puzzle shell", () => {
  ...
  assert.equal(state.current.length, 16);
  assert.equal(state.solution.length, 16);
  assert.equal(state.given.length, 16);
  assert.ok(state.current.includes(0));
  assert.equal(state.selectedIndex, state.current.findIndex((value) => value === 0));
  assert.equal(state.maxMistakes, 5);
});

test("4x4 placeNumber records mistakes on conflicts", () => {
  ...
  const rowStart = Math.floor(selectedIndex / 4) * 4;
  const conflictingValue = base.current.slice(rowStart, rowStart + 4).find((value) => value !== 0);
  ...
  assert.match(next.lastMessage, /错误 1\/5/);
});

test("4x4 giveHint fills the selected empty cell with the solution", () => {
  const base = engine.createNewGame({
    difficulty: "normal",
    now: 1000,
...
test("4x4 saveArchive and restoreGame keep progress intact", () => {
  ...
  assert.equal(restored.gameVersion, "4x4");
});
